    ...
    >
      <div className="container-pg max-w-4xl mx-auto">
        <div className="text-center mb-14">
          <p id="security-heading" className="text-sm font-medium text-[#1a1c1c] mb-4">
            Built with security in mind.
          </p>
          <p className="text-[#42493e] text-base max-w-xl mx-auto text-center">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {SECURITY_FEATURES.map((f) => (
            <div
              key={f.title}
              className={`
                p-6 rounded-2xl border border-[rgba(24,24,27,0.08)] bg-[#faf9f8]
                ${f.size === 'large' ? 'md:col-span-1' : ''}
              `}
            >
              <div className="w-8 h-8 rounded-lg bg-white border border-[rgba(24,24,27,0.08)] flex items-center justify-center mb-4">
                <SecurityIcon name={f.title} />
              </div>
...
function SecurityIcon({ name }: { name: string }) {
  const icons: Record<string, React.ReactNode> = {
    "Secure auth": (
      <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#154212" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
